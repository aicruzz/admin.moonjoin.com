                        ...
                        <div class="order-invoice-right mt-3 mt-sm-0">
                            <div class="btn--container ml-auto align-items-center justify-content-end">
                                ...
                                @if ( !$editing && in_array($order->order_status, ['pending', 'confirmed', 'processing', 'accepted']) &&
                                        ...
                                        $order->prescription_order == 0 && ($order->payment_method == 'cash_on_delivery' || $order->payment_method == 'wallet'))
                                        ...
                                        @if (isset($detail->item_id) && $detail->status)
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-right">
                                                    ...
                                                    <h5 class="mb-0">{{ \App\CentralLogics\Helpers::format_currency($amount) }}</h5>
                                                </td>
                                                {{-- Action column — only when editing --}}
                                                @if($editing)
                                                    <td class="text-center">
                                                        <button
                                                            class="btn btn-sm btn-outline-danger removeFromCart"
                                                            data-key="{{ $key }}"
                                                            title="{{ translate('messages.remove') }}">
                            ...
                            <div class="col-md-12">
                                <dl class="row text-right">
                                    ...
                                    <dt class="col-6 color-8a8a8a fs-12">{{ translate('messages.subtotal') }}
                                        ...
                                        @endif
                                        :</dt>
                                    <dd class="col-6 text-dark fs-14 d-flex align-items-center justify-content-end" style="gap:6px;">
                                        <i class="tio-edit color-8a8a8a"></i>
                                    ...
                                    <div class="text-center bg-light2 rounded p-xxl-20 p-3">
                                        ...
                                        <div class="btn--container justify-content-center">
                                            ...
                                            <button type="button" class="btn btn--primary btn-sm form-alert"
                                                    ...
                                                    data-message="{{ translate('The customerâ€™s digital payment has failed. Before switching this order to Cash on Delivery (COD), please confirm the payment issue with the customer to avoid any misunderstandings.') }}">
                    ...
                    <div class="modal-body pt-0">
                        ...
                        @if ($order?->offline_payments?->status != 'verified')
                            <div class="btn--container justify-content-end mt-xxl-5 mt-4 pt-xxl-1">
                                @if ($order?->offline_payments?->status != 'denied')
                                    <button type="button" class="btn btn--reset offline_payment_cancelation_note" data-toggle="modal" data-target="#offline_payment_cancelation_note" data-id="{{ $order['id'] }}" class="btn btn--reset">{{translate('Payment_didnâ€™t_Receive')}}</button>
                                @elseif ($order?->offline_payments?->status == 'denied')
                                ...
                                @else
                                        <button type="button" class="btn btn--primary btn-sm form-alert"
                                                ...
                                                data-image-url="{{ asset('public/assets/admin/img/tughrik.png') }}"
                                                data-title="{{ translate('Switch to Cash on Delivery?') }}"
                                                data-message="{{ translate('The customerâ€™s offline payment has failed. Before switching this order to Cash on Delivery (COD), please confirm the payment issue with the customer to avoid any misunderstandings.') }}">
