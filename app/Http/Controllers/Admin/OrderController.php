...
class OrderController extends Controller
{
    ...
    public function quick_view_cart_item(Request $request)
    {
        ...
        $active_cart_count = session('order_cart')->where('status', true)->count();

        return response()->json([
            'success' => 1,
            'view' => view('admin-views.order.partials._quick-view-cart-item', compact('order_id', 'product', 'cart_item', 'item_key', 'item_type', 'active_cart_count'))->render(),
        ]);
    }
    ...
    public function sendOrderWalletChangeNotification($user_id, $type, $amount, $reason, $order_id)
    {
        $notification_data = [
            'title' => translate('messages.wallet_update'),
            'description' => $reason . ' ' . Helpers::format_currency($amount) . ($type == 'credit' ? ' credited to' : ' debited from') . ' your wallet.',
            'order_id' => $order_id,
            'image' => '',
            'type' => 'wallet',
        ];
