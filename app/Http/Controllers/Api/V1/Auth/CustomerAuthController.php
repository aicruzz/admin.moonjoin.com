...
class CustomerAuthController extends Controller
{
    ...
    public function register(Request $request)
    {
        ...
        $user->save();
         ...
         try {
            app(NinePsbService::class)->createVirtualAccount($user);
        } catch (\Exception $ex) {
            info('Virtual account creation failed for user ' . $user->id . ': ' . $ex->getMessage());
        }
    ...
    private function manual_login($request_data){
        ...
        if (auth()->attempt($data)) {
            ...
            // Create virtual account for old users who don't have one
